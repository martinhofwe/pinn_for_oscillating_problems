...
class PhysicsInformedNN(object):
    def __init__(self, layers, h_activation_function, logger, simul_constants, domain, physics_scale, lr, data,
        ...
        self.physics_weights =  tf.Variable(tf.random.uniform([self.x_physics.shape[0], 1], dtype=self.dtype))
        self.data_weights =  tf.Variable(tf.random.uniform([self.x_ic.shape[0], 1], dtype=self.dtype))

        print("weight")
        print(self.physics_weights.shape)
    ...
    def calc_physics_loss(self, x_col): # changed to sqauring weights and loss before multiplying
        ...
        p_loss_mean = tf.reduce_mean(tf.square(self.physics_weights)*tf.square(p_loss))
        return p_loss_mean
    ...
    def train_step(self, x_col):
        ...
        grads_p = tape.gradient(combined_weighted_loss, self.physics_weights)
        grads_d = tape.gradient(combined_weighted_loss, self.data_weights)
        del tape
        ...
        self.optimizer_sa.apply_gradients(zip([-grads_d], [self.data_weights]))
