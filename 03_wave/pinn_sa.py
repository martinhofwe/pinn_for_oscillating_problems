...
class PINN:
    ...
    def loss_ini(self, t, x, y, u, weights_sa_ini):
        ...
        loss = tf.reduce_mean(tf.square(weights_sa_ini*(u - u_))) \
                + tf.reduce_mean(tf.square(u_t_)) # todo frage warum + tf.reduce_mean(tf.square(u_t_))
    ...
    def loss_glb(self, 
        ...
        loss_bnd = self.loss_bndx(t_bndx, x_bndx, y_bndx, weights_sa_bndx) \
                    + self.loss_bndy(t_bndy, x_bndy, y_bndy, weights_sa_bndy)
