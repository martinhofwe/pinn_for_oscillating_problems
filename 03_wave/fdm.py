...
def FDM(xmin, xmax, nx, dx, 
        ...
        x, y, u, c, BC):
    # FDM for reference
    u[0, :, :] = np.exp(-(x - 3) ** 2) * np.exp(-(y - 3) ** 2) # (x-3) sets x center position, (y-3) sets y center position
    u[1, :, :] = u[0, :, :]
    ...
    plt.imshow(u[0, :, :], cmap = "coolwarm", vmin = -.5, vmax = .5)
    ...
    ax.set_zlabel("u (x, y)", fontstyle = "italic")
    #plt.show()
    fig.savefig('fdm solutions.svg', format='svg', dpi=1200)
    return u
