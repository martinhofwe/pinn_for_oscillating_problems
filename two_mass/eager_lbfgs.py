# take from https://github.com/levimcclenny/SA-PINNs/blob/master/Allen-Cahn/eager_lbfgs.py
...
def lbfgs(opfunc, x, state, maxIter = 100, learningRate = 1, do_verbose = True):
  ...
  f, g, logger_data = opfunc(x)

  f_hist = [f]
  log_data = [logger_data]
  currentFuncEval = 1
  ...
  while nIter < maxIter:
    ...
    if do_verbose:
      if nIter % 100 == 0 or nIter == 0:
        #log_data = [m1_data_loss, m2_data_loss, f_pred_m1, f_pred_m2]
        print("Step %3d loss %6.8f m1 %6.8f  m2 %6.8f pm1 %6.8f  pm2 %6.8f"%(nIter, f.numpy(), (logger[1])[0].numpy(),(logger[1])[1].numpy(), (logger[1])[2].numpy(), (logger[1])[3].numpy()))
  ...
  return x, f_hist, currentFuncEval, log_data
