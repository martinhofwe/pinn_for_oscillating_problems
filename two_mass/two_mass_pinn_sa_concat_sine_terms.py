...
class PhysicsInformedNN(object):
    ...
    def f_model(self, x_):
        ...
        #   - (self.c / self.m) * y - (self.d / self.m) * y_dx - (y_dx2 / self.scaling_factor)
    ...
    def fit(self, training_epochs):
        ...
        for epoch in range(training_epochs):
            ...
            self.logger.log_train_epoch(self.logger.total_epoch, loss_value, log_data)

            if self.logger.total_epoch % 25_000 == 0:
                self.store_intermediate_result(self.logger.total_epoch, pred_parameters, physics_losses)
        self.logger.log_train_end(self.logger.total_epoch, log_data)
...
def main():
    ...
    terms_scale_lst = [1]
    if task_id == 0:
        terms_scale_lst = [0, 0.5, 1]
    if task_id == 1:
        terms_scale_lst = [0, 1, 1, 1, 1]
    if task_id == 2:
        terms_scale_lst = [0, 0.5, 0.5, 1, 1]
    act_func = "sine"
    af_str = "sin"
    ...
    experiment_name = "two_mass_sa_martin_tc_fixeds_conc_sin_terms_h_l_" + str(hidden_layers) + "_w_" + str(
