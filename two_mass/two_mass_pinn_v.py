...
class Logger(object):
  ...
  def log_train_end(self, epoch, custom=""):
    ...
    self.loss_over_meta.append([data_error_m1, data_error_m2])
...
class PhysicsInformedNN(object):
  def __init__(self, layers, h_activation_function, optimizer, logger,c1, d1, c2, d2,m1,m2, n_inputs=3,scaling_factor=1e12,physics_scale=1.00, p_norm="l2", ea_stopping=False):
...
def main():
 ...
 #   width = 256
  ...
  # parameters ########################################################################################################
  ppi = 0
  max_iter_overall = 2_000#1_200_000#
  ...
  experiment_name = "two_mass_vanilla_free_ppi_" + str(ppi) + "_frame_" + str(mode_frame) + "_h_l_" + str(hidden_layers) + "_w_" + str(width) + "_pn_" + p_norm + "_af_" + af_str + "_input_" + str(input_bool) + "_expl_" + str(exp_len) + "_ps_" + str(physics_scale_new) + "_pstart_" +str(p_start_step)+ "_wf_"+str(weight_factor)+"_ea_"+str(ea_stopping) + "_id_" + str(task_id)
  ...
  y_m2_simul, y_m2_dx_simul, y_m2_dx2_simul, y_m1_simul, y_m1_dx_simul, y_m1_dx2_simul, u_simul, up_simul, tExci, simul_constants = get_simulated_data_two_mass(start_vec, end_time=20, steps=4001, exp_len=exp_len, m1=m1, m2=m2, css=css,dss=dss, debug_data=True)
  ...
  pinn = PhysicsInformedNN(layers, h_activation_function= act_func, optimizer=tf_optimizer, logger=logger,c1=c1,d1=d1,c2=c2, d2=d2,m1=m1,m2=m2, n_inputs=layers[0],
                           scaling_factor=scaling_factor,physics_scale=physics_scale, p_norm=p_norm, ea_stopping=ea_stopping)
  ...
  for i in range(meta_epochs):
      ...
      ea_mean_lst, ea_median_lst, ea_mean_data_lst, ea_median_data_lst = pinn.fit(input_data, y_data_all,input_data_physics,input_data_semi, y_data_semi_pseudo, pseudo_physics_norm, tf_epochs, show_summary=show_summary)
