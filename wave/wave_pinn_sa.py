...
class PhysicsInformedNN(object):
  def __init__(self, layers, h_activation_function, optimizer, logger,velocity, n_inputs=3,scaling_factor=1e12,physics_scale=1.00, p_norm="l2"):
    ...
    for count, width in enumerate(layers[1:]):
        if width >1:
          if h_activation_function == "sine" and count == pot_sin_layer and False:
            assert False
            print("sine af")
    ...
    self.physics_weights = tf.Variable(tf.random.uniform([500, 1])) # todo not hard coded
    self.data_weights = tf.Variable(tf.random.uniform([500, 1]))  # todo not hard coded
    self.optimizer_data = tf.keras.optimizers.Adam(lr=0.005, beta_1=.90)
    self.optimizer_physics = tf.keras.optimizers.Adam(lr=0.005, beta_1=.90)
  ...
  @tf.function
  def __loss(self,x, y_lbl,x_physics, y,p_weight, d_weight, x_semi_begin=None, semi_scale=None, p_norm=None):
    ...
    if x_semi_begin is not None:
      ...
      data_loss = tf.reduce_mean((d_weight*(y_lbl - y))**2)
    if p_norm == "l2":
      physics_loss = tf.reduce_mean((p_weight*f_pred)**2)
    if p_norm == "l1":
      physics_loss = tf.reduce_mean(tf.math.abs(p_weight*f_pred))

    return data_loss+data_loss_semi+self.physics_scale*physics_loss
  
  def __grad(self, x, y_lbl, x_physics, x_semi_begin, semi_scale,  p_weight, d_weight):
    with tf.GradientTape(persistent=True) as tape:
      tape.watch(x)
      loss_value = self.__loss(x,y_lbl, x_physics, self.model(x), p_weight, d_weight, x_semi_begin, semi_scale, p_norm=self.p_norm)

      grads_data = tape.gradient(loss_value, d_weight)
      grads_p = tape.gradient(loss_value, p_weight)
  ...
  def f_model(self,input):
    ...
    with tf.GradientTape(persistent=True) as tape:
      ...
      tape.watch(t)
      tape.watch(x)
      input_watched_full = tf.concat([t, x], axis=-1)
      # Determine residual
    ...
    return u_dx2-(u_dt2/(self.velocity**2))
  ...
  def fit(self, x, y_lbl,x_physics,x_semi=None, y_semi=None, semi_scale=None, tf_epochs=5000, show_summary=False):
    ...
    for epoch in range(tf_epochs):
      loss_value, grads, grads_data, grads_p = self.__grad(x, y_lbl, x_physics, x_semi_begin, semi_scale, p_weight=self.physics_weights, d_weight=self.data_weights)
      self.optimizer.apply_gradients(zip(grads, self.__wrap_training_variables()))
      self.optimizer_data.apply_gradients(zip([-grads_data], [self.data_weights]))
      self.optimizer_physics.apply_gradients(zip([-grads_p], [self.physics_weights]))
...
def main():
  ...
  layers = [2] # input time and position x
  ...
  if width == 1024:
    ...
    meta_epochs = 800_000
  lr = tf.Variable(1e-5)#tf.Variable(1e-4)
  ...
  experiment_name = "wave_sa_ppi_" + str(ppi) + "_frame_" + str(mode_frame) + "_h_l_" + str(hidden_layers) + "_w_" + str(width) + "_pn_" + p_norm + "_af_" + af_str + "_input_" + str(input_bool) + "_expl_" + str(exp_len) + "_ps_" + str(physics_scale_new) + "_pstart_" +str(p_start_step) + "_id_" + str(task_id)
  ...
  y_data = wavefields_flat[data_start:num_points+1:data_sampling]*scaling_factor
  y_lbl = wavefields_flat[data_start:]
  t = time_steps[data_start:]
  locations = np.arange(0, wavefields_flat.shape[1])
  ...
  y_lbl_error = wavefields_flat[considered_steps].flatten()
  # plotting
  ...
  def error():
    ...
    y, f = pinn.predict_multiple_images(considered_steps=considered_steps, locations=locations)
    data_error = np.mean((y_lbl_error - y) ** 2) #to do check flattened correctly?
  ...
  y_pred, f_pred = pinn.predict_multiple_images(considered_steps=considered_steps, locations=locations)
  ...
  for i in range(meta_epochs):
      ...
      idx_pos = np.expand_dims(random.choices(np.arange(0, locations.shape[0]), k=batch_size), 1)
      idx_time = np.expand_dims(random.choices(np.arange(0, x_data.shape[0]), k=batch_size), 1)

      batch_pos = locations[idx_pos]
      batch_time = x_data[idx_time]



      input_data = tf.concat([batch_time, batch_pos], axis=-1) # timestep and position
      data_lbl = y_data[idx_time, idx_pos]



      # create physics loss batch
      idx_pos_p = np.expand_dims(random.choices(np.arange(0, locations.shape[0]), k=5), 1) # todo needs rework? more points?
      idx_time_p = np.expand_dims(np.arange(p_start_step, time_steps.shape[0]), 1)


      batch_pos_p = np.tile(locations[idx_pos_p], (100, 1))
      batch_time_p = np.expand_dims(np.repeat(time_steps[idx_time_p], 5), 1)

      input_data_physics = tf.concat([batch_time_p, batch_pos_p], axis=-1)
  ...
  y_pred, f_pred = pinn.predict_multiple_images(considered_steps=np.array([0, 2, 4, 6, 8, 10, 12, 14, 16]), locations=locations)
