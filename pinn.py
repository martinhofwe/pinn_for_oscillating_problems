...
class PhysicsInformedNN(object):
  def __init__(self, layers, optimizer, logger,c,d,n_inputs=3,scaling_factor=1e12,physics_scale=1.0):
  ...
  @tf.function
  def __loss(self,x, y_lbl,x_physics, y, x_semi_begin=None, semi_scale=None):
  ...
  def __grad(self, x, y_lbl, x_physics, x_semi_begin, semi_scale):
    with tf.GradientTape() as tape:
      tape.watch(x)  
      loss_value = self.__loss(x,y_lbl, x_physics, self.model(x), x_semi_begin, semi_scale)
...
def main():
  ...
  experiment_name = "ppi_" + str(ppi) + "_frame_" + str(mode_frame)
  ...
  pinn = PhysicsInformedNN(layers, tf_optimizer, logger,c=c,d=d,n_inputs=layers[0],
